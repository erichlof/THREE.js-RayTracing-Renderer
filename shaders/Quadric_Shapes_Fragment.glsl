...
struct Material { int type; vec3 color; vec3 color2; float roughness; float IoR; int textureID; };
...
//---------------------------------------------------------------------------------------
{
	...
	if (d > 0.0 && d < t)
	{
		...
		intersectionUV = calcUnitCylinderUV(intersectionPoint) * hyperbolicParaboloids[0].uvScale;
	}
...
//-------------------------------------------------------------------------------------------
{
	float pointLightPower = 4.0;
        Material pointLightMaterial = Material( POINT_LIGHT, vec3(1.0) * pointLightPower, vec3(1.0), 0.0, 0.0, -1 );
	...
	int sceneUsesDirectionalLight = TRUE;//FALSE;
	//int previousIntersectionMaterialType;
	//intersectionMaterial.type = -100;

	// For the kind of ray tracing we're doing, 6 or 7 bounces is enough to do all the reflections and refractions that are most noticeable.
	...
	for (int bounces = 0; bounces < 8; bounces++)
	{
		...
		{
			if (mod(floor(intersectionUV.x) + floor(intersectionUV.y), 2.0) == 1.0)
				intersectionMaterial.color = intersectionMaterial.color2;
...
//-----------------------------------------------------------------------
{
	...
	Material uvGridMaterial = Material(CLEARCOAT_DIFFUSE, vec3(1.0), vec3(1.0), 0.0, 1.3, 0 );
	Material redMaterial = Material(PHONG, vec3(0.7, 0.01, 0.01), vec3(1.0), 1.0, 0.0, -1 );
	Material metalMaterial = Material(METAL, vec3(1.000, 0.766, 0.336), vec3(1.0), 0.0, 0.0, -1 );
	Material glassMaterial = Material(TRANSPARENT, vec3(0.4, 1.0, 0.6), vec3(1.0), 0.0, 1.5, -1 );
	Material grayBlackCheckerMaterial = Material(CHECKERED_CLEARCOAT, vec3(0.5), vec3(0.001), 0.0, 1.4, -1 );
	Material whiteRedCheckerMaterial = Material(CHECKERED_CLEARCOAT, vec3(0.8), vec3(0.5, 0.01, 0.01), 0.0, 1.4, -1 );
	// vec3 pointLightPosition = vec3(4, 10, 5);
	...
	spheres[0] = UnitSphere(vec2(1, 1), redMaterial);
	spheres[1] = UnitSphere(vec2(1, 1), metalMaterial);
	spheres[2] = UnitSphere(vec2(2, 1), uvGridMaterial);
	spheres[3] = UnitSphere(vec2(1, 1), glassMaterial);

	cylinders[0] = UnitCylinder(vec2(2, 1), uvGridMaterial);
	cylinders[0].material.IoR = 1.3; // decrease the shininess just a bit

	cones[0] = UnitCone(vec2(6, 4), whiteRedCheckerMaterial);
	paraboloids[0] = UnitParaboloid(vec2(6, 4), whiteRedCheckerMaterial);
	hyperboloids[0] = UnitHyperboloid(vec2(6, 4), whiteRedCheckerMaterial);
	hyperbolicParaboloids[0] = UnitHyperbolicParaboloid(vec2(6, 4), whiteRedCheckerMaterial);
}
